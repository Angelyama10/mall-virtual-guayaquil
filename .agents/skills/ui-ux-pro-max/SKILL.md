...
The search script lives inside this project-local skill directory. Resolve `<skill-dir>` to the directory containing this `SKILL.md` file and invoke the script from there. Do not use `CLAUDE_PLUGIN_ROOT`; that variable belongs to Claude plugins and is not available in Codex.

```bash
python3 "<skill-dir>/scripts/search.py" "<query>" --domain <domain>
```
...
```bash
python3 "<skill-dir>/scripts/search.py" "<query>" --design-system --variance <1-10> --motion <1-10> --density <1-10>
```
...
**Example:**
```bash
python3 "<skill-dir>/scripts/search.py" "internal analytics dashboard" --design-system --variance 8 --motion 7 --density 8 -p "Ops Console"
```
...
# Step 4: stack guidelines
python3 "<skill-dir>/scripts/search.py" "suspense streaming bundle" --stack nextjs
```
